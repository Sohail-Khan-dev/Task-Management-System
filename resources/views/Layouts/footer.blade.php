<!-- Footer -->
<div class="bg-blue-500 text-white">
    <div class="container mx-auto px-6 pt-6 pb-2">
        <div class="flex justify-between">
            <!-- Footer Column 1 -->
            <div class="w-full md:w-1/4 text-sm mb-6 md:mb-0">
                <h5 class="uppercase mb-4 font-bold">Company</h5>
                <ul class="list-none">
                    <li class="mb-2">
                        <a href="#about" class="hover:text-blue-200">About Us</a>
                    </li>
                    <li class="mb-2">
                        <a href="#contact" class="hover:text-blue-200">Contact Us</a>
                    </li>
                    <li class="mb-2">
                        <a href="#team" class="hover:text-blue-200">Team</a>
                    </li>
                    <li class="mb-2">
                        <a href="#careers" class="hover:text-blue-200">Careers</a>
                    </li>
                </ul>
            </div>

            <!-- Footer Column 2 -->
            <div class="w-full md:w-1/4 text-sm mb-6 md:mb-0">
                <h5 class="uppercase mb-4 font-bold">Resources</h5>
                <ul class="list-none">
                    <li class="mb-2">
                        <a href="#blog" class="hover:text-blue-200">Blog</a>
                    </li>
                    <li class="mb-2">
                        <a href="#help" class="hover:text-blue-200">Help Center</a>
                    </li>
                    <li class="mb-2">
                        <a href="#privacy" class="hover:text-blue-200">Privacy Policy</a>
                    </li>
                    <li class="mb-2">
                        <a href="#terms" class="hover:text-blue-200">Terms of Service</a>
                    </li>
                </ul>
            </div>

            <!-- Footer Column 3 (Optional) -->
            <!-- Add more columns as needed -->

            <!-- Footer Social Links (Optional) -->
            <div class="w-full md:w-1/4 text-sm mb-6 md:mb-0">
                <h5 class="uppercase mb-4 font-bold">Follow Us</h5>
                <!-- Social Media Links Here -->
            </div>
        </div>

        <div class="border-t border-gray-700 mt-8 pt-6 text-center">
            <p class="text-sm">
                &copy; 2024 Task Management System. All rights reserved.
            </p>
        </div>
    </div>
</div>
